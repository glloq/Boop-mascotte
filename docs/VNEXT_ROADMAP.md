...
| VNX-22 | ✅ **Pose editor**, on a new representation. `docs/HAND_REPRESENTATIONS_STUDY.md` measured what the single outline could not draw (no side view, no finger lines, a flip that collapsed halfway) and the hand became **six parts drawn from tables of numbers** as a cartoon glove, with a **facing axis** (palm · side · far side) stored as pose grids the way the head turns, a pose as a parameter driving a key on every part it moves, a **pose editor** (sliders per digit, a preview from the same generator, *Touch the thumb*, Capture as one command) and **sets of drawings** for artwork of the author's own. Creating and removing a pose live on one surface, which closes the asymmetry `VNEXT_COMPONENTS.md` recorded. No runtime concept was added; `runtime/hands.js` learned to carry a drawing with the hand and to share the hand between several (`docs/HAND_RIGGING.md`) |
