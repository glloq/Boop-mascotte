...
| **M3** | Head + hands UX | VNX-16 → 24 | VNX-16, 17, 19, 20, 21 done; 18 deferred with a reason; 22 superseded by the layered hand set; **23 → 24** unparked |
...
| VNX-22 | ✅ **Delivered by the layered hand set**, not as a pose editor. The pose editor was built on a hand made of six deforming parts with a pseudo-3D facing axis; that hand is gone. A hand is still a **static drawing** chosen by name — but a drawing is a group of named layers read from a file now, so there **is** something inside one to edit, and a ninth is a ninth file (`docs/HAND_STYLES.md`, "A gesture is a file"). What VNX-22 asked for is not a pose editor: it is Edit Shape on a layer, which already exists. **Create ▸ Hands** is where a set is added to and imported; the drawings of a hand are edited under the hand in the Character Builder |
| VNX-23 | ◐ unblocked by VNX-22. A reaction raises `gestures` (a side and a pose) beside its expression and its clip, and the reaction editor offers every pose the hand has — captured ones and drawings included, since a pose is a parameter whatever moves it. What is left is the gesture *timeline*: a pose held for part of a reaction rather than its whole envelope |
