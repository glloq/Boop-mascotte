...
| VNX-22 | ⤫ **Superseded**, and the fresh look is taken. The pose editor was built on a hand made of six deforming parts with a pseudo-3D facing axis; that hand is gone. A hand is still a **static drawing** chosen by name — but a drawing is a group of named layers read from a file now, so there **is** something inside one to edit, and a ninth is a ninth file (`docs/HAND_STYLES.md`, "A gesture is a file"). What VNX-22 asked for is not a pose editor: it is Edit Shape on a layer, which already exists |
