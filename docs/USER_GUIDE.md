...
1. **Start on Home.** Pick **New Character** to dress a face from the library in the Character Builder — a preset, then any part swapped for another style, the hands too, under a minute (`docs/CHARACTER_BUILDER.md`) — or **Mascot Face** — the one template, a complete cartoon face that arrives rigged, blinking and turning in 2.5D (`docs/MASCOT_TEMPLATE.md`) — or **Blank canvas** to draw your own mascot from nothing with the shape tools (Pen with curves, Line, Rectangle, Ellipse, Polygon or Star, Text; the bar under the toolbar sets fill, stroke, sides, the grid and snapping), or **Import SVG** for artwork only, or **Open Project** for a previously saved complete project. **Build a face** — pick a head, eyes and a mouth — is the third card on Home.
