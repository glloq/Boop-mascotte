...
4. **Add a Nod.** Open **Animate** and **Add** a motion preset (Nod, Shake): it plays once, and the Inspector tunes amplitude, duration, repeats and loop. **Open in Timeline** shows the same animation key by key; for anything more complex, create a clip there, enable Auto Key, move the playhead and pose the mascot.
