...
> Checked against the code at `f5b4945` (2026-09-19). What is here was verified
> line by line; what was found to be out of date has been corrected in place and
> the correction is noted. `docs/CURRENT_STATE.md` is the authoritative
> description of what the editor does, and this file is the short list of what
...
- There is no F-curve/tangent editor, timeline-style multi-clip mixer (motions layer at runtime but there is no track view for them), bones, physics, audio timeline, or 3D runtime.
- A **reaction** can carry conditions — *only if* a movement compares to a number, or the mascot is in a state, joined with `and` (`docs/V4_ROADMAP.md`, Phase 9). A **transition** cannot: the state machine still moves between poses on demand rather than when something becomes true. A rig using reaction conditions asks for `reaction:condition` in `requires`, so a runtime without them declines it rather than firing every reaction unconditionally.
- **Mesh deformation** applies to pictures only, at 3×3 or 4×4, one mesh per picture, driven by at most one parameter (`docs/V4_ROADMAP.md`, Phase 7). A path is deformed by warps, keyforms and shape keys instead. Nothing bends a group as a whole.
- Pictures import as **PNG or WebP**; JPEG is refused because a mascot piece needs an alpha channel. The longest side is limited to 8192 px.
- **Shape keys, deformers and depth/parallax have no authoring surface**: the runtime plays them and an imported rig brings them, and Advanced → **Deformation** lists what a project carries, read-only. Warps, meshes and pins *do* have one — warps and meshes are dragged on the canvas, pins are placed and dragged on it with reach handles — so the shorter sentence that used to be here ("only warps and keyforms have an authoring panel") understated what exists and blurred what is actually missing. Re-editing an existing shape key means switching a movement's method to force a capture session; **attachment points are moved by typing X and Y** and are not drawn on the canvas at all.
- The state graph is laid out by hand: a node's position is authored data, saved with the project and undoable, with pan, zoom, marquee selection, groups, notes and a one-press auto-arrange (`docs/V4_ROADMAP.md`, Phase 10). **It is drawn in the canvas column** (`shell/canvas-column.js`), sized `off` / `small` / `large` — the sentence that used to be here said it lived in a 260 px sidebar and that "the diagram wants the canvas", which stopped being true when the board took the canvas column (`docs/BEHAVIOR_STUDIO.md`). Transitions interpolate States and do not trigger Timeline clips.
