...
- The state graph is laid out by hand: a node's position is authored data, saved with the project and undoable, with pan, zoom, marquee selection, groups, notes and a one-press auto-arrange (`docs/V4_ROADMAP.md`, Phase 10). It is drawn in the left sidebar column, about 260 px wide, so a four-state machine fits at just under half size — the geometry is sized for that, but the diagram wants the canvas. Transitions interpolate States and do not trigger Timeline clips.
