...
- State transitions graph (editable allowed transitions per state).
- Preview state simulator: state chips now respect transition graph rules and show allowed/blocked transitions.
- Preview mode with sliders and live binding application.
- Preview pipeline now compiles a full frame (transforms + path morph updates) before applying to canvas for cleaner phase-3 performance evolution.
...
- Global + per-state constraints scaling for translate/rotate/scale (useful for emotion-specific motion intensity).
- Runtime now uses a lightweight safe expression parser (no `new Function`) and caches transforms to reduce DOM writes.
- Runtime transition guard blocks invalid state changes when transitions are configured.
...
- Face Builder: generate a starter face SVG from head/eyes/mouth options in one click.
...
- Rig import workflow: load existing `rig.json` back into editor to continue iteration on parameters/states/transitions.
- Export:
  - `mascot.svg`
  - `rig.json`
