...
test('the pad moves the head the way it is dragged', async ({ page }) => {
  ...
  // 141 transform records plus the 19 depth ones a projected turn writes
  ...
  // mouth grew a lower row of teeth, a tongue tip and the crease down it, and
  // the generator writes seven channels for each drawing that turns
  // (docs/MOUTH_BUILD.md).
  expect((await documentOf(page)).keyforms.length).toBe(178);
});
