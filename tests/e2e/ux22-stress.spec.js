...
test('@stability a long project (60 expressions, 75 motions, 40 reactions, 23 states) keeps the budgets', async ({ page }) => {
  const errors = [];
  ...
  expect(exportMs).toMatchObject({ reactions: 40, animations: 75, expressions: 60 });
  ...
  // The template's blink keeps one loop awake by design; what must stay bounded
  // is the number of loops.
  await expect.poll(() => diagnostics(page).then((d) => d.preview.activeRaf)).toBeLessThanOrEqual(1);
