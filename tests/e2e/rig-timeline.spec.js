...
import { goToAnimate, openFaceBuilder, openFreshEditor, selectSemanticPartById } from './editor-helpers.js';
...
async function load(page, name) {
  if (name === 'basic') await page.getByRole('button',{name:'Start with Basic Face',exact:true}).click();
  else { const examples=page.locator('.create-tools > details.more-examples'); if (!(await examples.getAttribute('open'))) await page.getByText('More templates',{exact:true}).click(); await page.locator(`#empty-${name}`).click(); }
  await expect(page.locator('#canvas svg svg')).toBeVisible();
}
async function part(page, name) { await page.getByRole('button',{name:'Rig',exact:true}).click(); await page.getByRole('button',{name,exact:true}).click(); }
...
test('semantic methods, roles, calibration, morph ownership and controls survive Save/Open',async({page})=>{
  await load(page,'talking');const before=await state(page);const download=page.waitForEvent('download');await page.getByRole('button',{name:'Save Project'}).click();const file=await download,path=await file.path();await page.getByRole('button',{name:'Create',exact:true}).click();await page.getByText('More templates',{exact:true}).click();await page.locator('#empty-basic').click();await page.locator('#project-file').setInputFiles(path);await expect.poll(()=>state(page).then(s=>s.semanticParts.mouth.controlDrivers.mouthOpen.method)).toBe('morph');const after=await state(page);expect(after.semanticParts.mouth.roles).toEqual(before.semanticParts.mouth.roles);expect(after.elements.mouth.morph).toEqual(before.elements.mouth.morph);await part(page,'Pupils / Gaze');const old=await page.locator('#pupilLeft').getAttribute('transform');await dragPad(page,.85,.3);await expect.poll(()=>page.locator('#pupilLeft').getAttribute('transform')).not.toBe(old);await setLive(page,'mouthOpen',1);await expect(page.locator('#mouth')).toHaveAttribute('d',after.elements.mouth.morph.pathB);
});

async function newLookClip(page){await load(page,'basic');await goToAnimate(page);await page.locator('#new-clip').click();await page.locator('#clip-name').fill('Gaze Test');await page.locator('#clip-name').dispatchEvent('change');await page.locator('#clip-duration').fill('1');await page.locator('#clip-duration').dispatchEvent('change');await page.locator('#track-param').selectOption('lookX');await page.locator('[data-action="add-track"]:visible').first().click();}
...
test('@critical Build a Face generates an honest valid project that previews and saves',async({page})=>{const errors=monitor(page);await openFaceBuilder(page);await page.getByRole('button',{name:'Build a Face',exact:true}).click();await page.locator('#face-head').selectOption('square');await page.locator('#face-eyes').selectOption('dot');await page.locator('#face-mouth').selectOption('sad');await page.locator('#generate-face').click();for(const id of ['head','eyeLeft','eyeRight','pupilLeft','pupilRight','browLeft','browRight','mouth'])await expect(page.locator(`#canvas svg svg #${id}`)).toBeVisible();const model=await state(page);expect(Object.keys(model.semanticParts)).toEqual(['head','eyes','gaze','eyebrows','mouth']);await selectSemanticPartById(page,'gaze');const before=await page.locator('#pupilLeft').getAttribute('transform');await dragPad(page,.8,.3);await expect.poll(()=>page.locator('#pupilLeft').getAttribute('transform')).not.toBe(before);await page.getByLabel('More project actions').click();await page.getByText('Advanced',{exact:true}).click();await page.getByRole('button',{name:'Problems'}).click();await expect(page.locator('#problems-panel')).toHaveAttribute('data-project-check-status','ready');await expect(page.locator('#problems-panel .manager-card')).toHaveCount(0);await page.getByRole('button',{name:/Preview/}).click();const download=page.waitForEvent('download');await page.getByRole('button',{name:'Save Project'}).click();await download;expect(errors).toEqual([]);});
