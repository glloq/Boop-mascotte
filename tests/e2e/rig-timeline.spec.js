...
test('a morph is refused on a path another movement already shapes, and says which',async({page})=>{
  // A shape key wins over a morph on the same outline (`compileRigFrame` emits
  // the summed path and the morph is never read), so a movement switched to
  // Morph there would silently stop working. The lids carry Narrow and Lid
  // curve as shapes of their own (docs/FACE_SVG_STATES.md), so this is the
  // clash an author meets first.
...
test('method switching preserves manual bindings and cleans only owned metadata',async({page})=>{
  ...
  // Refused, and Open is left on the method it was actually on -- the transform
  // it was moved to above, to get the outline out of Smile's way.
  ...
  expect(faceKeys(model)).toEqual(['lidUpperLeft-eyeSquint','lidUpperLeft-eyeCurve','lidLowerLeft-eyeSquint','lidLowerLeft-eyeCurve','lidUpperRight-eyeSquint','lidUpperRight-eyeCurve','lidLowerRight-eyeSquint','lidLowerRight-eyeCurve','creaseUpperLeft-eyeSquint','creaseUpperLeft-eyeCurve','creaseLowerLeft-eyeSquint','creaseLowerLeft-eyeCurve','creaseUpperRight-eyeSquint','creaseUpperRight-eyeCurve','creaseLowerRight-eyeSquint','creaseLowerRight-eyeCurve','mouth-skull','uvula-skull','teeth-skull','teethLower-skull','tongue-skull','tongueTip-skull','tongueGroove-skull','teeth-show','teethLower-show','tongue-show','uvula-show','tongueTip-out','tongueGroove-out','tongue-out','tongueTip-curl','tongueGroove-curl','head-jaw'],'the teeth, the tongue and the jaw belong to their own controls, the lids to theirs, and the head-follow to none');
