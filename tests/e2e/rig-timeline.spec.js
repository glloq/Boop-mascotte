...
test('method switching preserves manual bindings and cleans only owned metadata',async({page})=>{
  ...
  expect(model.shapeKeys.map(key=>key.id)).toEqual(['mouth-open','mouth-smile','mouth-frown']);
  ...
  expect(model.shapeKeys.map(key=>key.id)).toEqual(['mouth-open']);
  ...
  expect(model.shapeKeys).toEqual([]);
