...
test('@critical Build a Face generates an honest valid project that previews and saves',async({page})=>{const errors=monitor(page);await page.locator('#face-head').selectOption('square');await page.locator('#face-eyes').selectOption('dot');await page.locator('#face-mouth').selectOption('sad');await page.locator('#generate-face').click();for(const id of ['head','eyeLeft','eyeRight','pupilLeft','pupilRight','browLeft','browRight','mouth'])await expect(page.locator(`#canvas svg svg #${id}`)).toBeVisible();const model=await state(page);expect(Object.keys(model.semanticParts)).toEqual(['head','eyes','gaze','eyebrows','mouth']);await part(page,'Pupils / Gaze');const before=await page.locator('#pupilLeft').getAttribute('transform');await dragPad(page,.8,.3);await expect.poll(()=>page.locator('#pupilLeft').getAttribute('transform')).not.toBe(before);page.once('dialog',async d=>{expect(d.message()).toContain('Valid');await d.accept();});await page.getByRole('button',{name:'Validate'}).click();await page.getByRole('button',{name:/Preview/}).click();const download=page.waitForEvent('download');await page.getByRole('button',{name:'Save Project'}).click();await download;expect(errors).toEqual([]);});
