...
async function newLookClip(page){await load(page,'basic');await page.getByRole('button',{name:'Animate',exact:true}).click();await page.locator('#new-clip').click();await page.locator('#clip-name').fill('Gaze Test');await page.locator('#clip-name').dispatchEvent('change');await page.locator('#clip-duration').fill('1');await page.locator('#clip-duration').dispatchEvent('change');await page.locator('#track-param').selectOption('lookX');await page.locator('#add-track').click();}
...
test('@critical Build a Face generates an honest valid project that previews and saves',async({page})=>{const errors=monitor(page);await page.getByText('More templates',{exact:true}).click();await page.getByText('Face Builder',{exact:true}).click();await page.locator('#face-head').selectOption('square');await page.locator('#face-eyes').selectOption('dot');await page.locator('#face-mouth').selectOption('sad');await page.locator('#generate-face').click();for(const id of ['head','eyeLeft','eyeRight','pupilLeft','pupilRight','browLeft','browRight','mouth'])await expect(page.locator(`#canvas svg svg #${id}`)).toBeVisible();const model=await state(page);expect(Object.keys(model.semanticParts)).toEqual(['head','eyes','gaze','eyebrows','mouth']);await part(page,'Pupils / Gaze');const before=await page.locator('#pupilLeft').getAttribute('transform');await dragPad(page,.8,.3);await expect.poll(()=>page.locator('#pupilLeft').getAttribute('transform')).not.toBe(before);await page.getByLabel('More project actions').click();await page.getByText('Advanced',{exact:true}).click();await page.getByRole('button',{name:'Problems'}).click();await expect(page.locator('#problems-panel')).toContainText('Mascot ready');await page.getByRole('button',{name:/Preview/}).click();const download=page.waitForEvent('download');await page.getByRole('button',{name:'Save Project'}).click();await download;expect(errors).toEqual([]);});
