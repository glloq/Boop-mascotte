...
import { enterFaceBuilder, goToAnimate, goToPreview, openFreshEditor, openGazeControl, setRangeControl, startBasicFace } from './editor-helpers.js';
...
async function part(page, name) { await page.locator('.workspace-tab[data-workspace="rig"]').click(); await page.getByRole('button',{name,exact:true}).click(); }
...
function parseTransformTranslation(value='') { const matrix=value.match(/matrix\([^, ]+[, ]+[^, ]+[, ]+[^, ]+[, ]+[^, ]+[, ]+([^, ]+)[, ]+([^\) ]+)/);if(matrix)return {x:Number(matrix[1]),y:Number(matrix[2])};const translate=value.match(/translate\(([^, )]+)(?:[, ]+([^\) ]+))?/);return translate?{x:Number(translate[1]),y:Number(translate[2]||0)}:{x:0,y:0}; }
...
test('Head calibration and controls update the real SVG transform',async({page})=>{
  ...
  await page.getByRole('button',{name:'Calculate'}).click();const center=await head.getAttribute('transform');await setRangeControl(page.locator('[data-control="headX"]'), .9);await expect.poll(()=>head.getAttribute('transform')).not.toBe(center);
...
test('@critical Build a Face generates an honest valid project that previews and saves',async({page})=>{const errors=monitor(page);await enterFaceBuilder(page);await page.locator('#face-head').selectOption('square');await page.locator('#face-eyes').selectOption('dot');await page.locator('#face-mouth').selectOption('sad');await page.locator('#generate-face').click();for(const id of ['head','eyeLeft','eyeRight','pupilLeft','pupilRight','browLeft','browRight','mouth'])await expect(page.locator(`#canvas svg svg #${id}`)).toBeVisible();const model=await state(page);expect(Object.keys(model.semanticParts)).toEqual(['head','eyes','gaze','eyebrows','mouth']);const before=await page.locator('#pupilLeft').getAttribute('transform');await moveGaze(page,.8);await expect.poll(()=>page.locator('#pupilLeft').getAttribute('transform')).not.toBe(before);await page.getByLabel('More project actions').click();await page.getByText('Advanced',{exact:true}).click();await page.getByRole('button',{name:'Problems'}).click();await expect(page.locator('#problems-panel')).toHaveAttribute('data-project-check-status','ready');await expect(page.locator('#problems-panel .manager-card')).toHaveCount(0);await goToPreview(page);const download=page.waitForEvent('download');await page.getByRole('button',{name:'Save Project'}).click();await download;expect(errors).toEqual([]);});
test('@critical timeline project metadata persists and remains playable after reload',async({page})=>{
  await newLookClip(page);await addKey(page,0,-1);await addKey(page,1,1);await page.locator('[data-key="lookX|1"]').click();await page.locator('[data-key-edit="easing"]').selectOption('easeInOut');const before=(await state(page)).animationEditor.activeClipId;const download=page.waitForEvent('download');await page.getByRole('button',{name:'Save Project'}).click();const path=await (await download).path();await page.locator('#project-file').setInputFiles(path);const restored=await state(page),clip=restored.animationClips.find(c=>c.name==='Gaze Test');expect(clip.tracks.lookX[1]).toMatchObject({time:1,value:1,easing:'easeInOut'});expect(restored.animationEditor.activeClipId).toBe(before);await page.locator('#playhead').fill('.25');await page.locator('#playhead').dispatchEvent('input');await expect.poll(()=>page.evaluate(()=>window.__BOOP_E2E__.effectiveParams().lookX)).toBeCloseTo(-.75);await expect.poll(async()=>parseTransformTranslation(await page.locator('#pupilLeft').getAttribute('transform'))).not.toEqual({x:0,y:0});
});

test('@critical @smoke cross-browser template, Rig, Timeline, Save and Export',async({page})=>{
  const errors=monitor(page);await load(page,'expressive');await moveGaze(page,.8);await expect(page.locator('#pupilLeft')).toHaveAttribute('transform',/translate/);await goToAnimate(page);await page.locator('#clip-play').click();await expect.poll(async()=>Number(await page.locator('#current-time').textContent())).toBeGreaterThan(0);await page.locator('#clip-pause').click();const saved=page.waitForEvent('download');await page.getByRole('button',{name:'Save Project'}).click();await saved;await page.getByRole('button',{name:'Export',exact:true}).click();for(const name of ['mascot.svg','rig.json','runtime.js']){const download=page.waitForEvent('download');await page.getByRole('button',{name:`Download ${name}`}).click();expect((await download).suggestedFilename()).toBe(name);}expect(errors).toEqual([]);
});
