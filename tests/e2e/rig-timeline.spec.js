...
test('Eye Open morph preserves closed-zero/open-one orientation on real paths',async({page})=>{
  const errors=monitor(page);await load(page,'expressive');await part(page,'Eyelids','controls');await page.locator('[data-method="eyeOpen"]').selectOption('morph');await page.locator('[data-rig-tab="calibrate"]').click();
  const closed='M 61 100 Q 85 100 109 100',open='M 61 100 Q 85 78 109 100',eye=page.locator('#upperLidLeft');
  await captureMorph(page,'eyeOpen','closed','upperLidLeft',closed);
  await captureMorph(page,'eyeOpen','open','upperLidLeft',open);
...
test('method switching preserves manual bindings and cleans only owned metadata',async({page})=>{
  ...
  await page.locator('[data-method="mouthOpen"]').selectOption('morph');model=await state(page);expect(model.elements.mouth.bindings.scaleY).toBeUndefined();expect(model.elements.mouth.bindings.opacity.expression).toBe('.5');expect(model.elements.mouth.morph.generatedBy.control).toBe('smile');
...
test('binding conflicts warn and preserve the existing owner',async({page})=>{
  const errors=monitor(page);await load(page,'basic');await part(page,'Mouth','controls');await page.locator('[data-method="mouthOpen"]').selectOption('morph');await page.evaluate(()=>window.__BOOP_E2E__.mutate(s=>{s.elements.mouth.bindings.scaleY={enabled:true,expression:'manual'};}));
...
test('semantic methods, roles, calibration, morph ownership and controls survive Save/Open',async({page})=>{
  await load(page,'talking');const before=await state(page);const download=page.waitForEvent('download');await page.getByRole('button',{name:'Save Project'}).click();const file=await download,path=await file.path();await openProjectMenu(page);await page.getByRole('button',{name:'New Project',exact:true}).click();await page.locator('[data-home] [data-template-id="basic"]').click();await page.locator('#project-file').setInputFiles(path);await expect.poll(()=>state(page).then(s=>s.semanticParts.mouth.controlDrivers.mouthOpen.method)).toBe('morph');const after=await state(page);expect(after.semanticParts.mouth.roles).toEqual(before.semanticParts.mouth.roles);expect(after.elements.mouth.morph).toEqual(before.elements.mouth.morph);const lookX=await openGazeControl(page);const old=await page.locator('#pupilLeft').getAttribute('transform');await setRangeControl(lookX, .85);await expect.poll(()=>page.locator('#pupilLeft').getAttribute('transform')).not.toBe(old);await setLive(page,'mouthOpen',1);await expect(page.locator('#mouth')).toHaveAttribute('d',after.elements.mouth.morph.pathB);
});
