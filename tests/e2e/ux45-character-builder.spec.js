...
test('@critical every drawing can be offered past the kind of face, and says which kind it is for', async ({ page }) => {
  ...
  // Named for what an author is making, not for the anatomy (UI-REDESIGN-03):
  // the id is still `mouth.animal-smile`, and the badge says `Animal`.
  await expect(muzzle.locator('.part-style-other')).toContainText('Animal');
...
test('@critical Type says what kind of face this is, offers only the kinds the library can draw, and changes nothing on the mascot', async ({ page }) => {
  ...
  // (UI-REDESIGN-03). The ids are load-bearing and do not move; only the words
  // an author reads are theirs: nobody shops for a snout.
  await expect(kind.locator('option[value="muzzle"]')).toContainText('Animal');
  await expect(kind.locator('option[value="beak"]')).toContainText('Bird');
  await expect(kind.locator('option[value="monster"]')).toContainText('Creature');
 ...
 */
test('@critical Style redraws the parts drawn in it, keeps every other one, and can be changed back', async ({ page }) => {
  await openFreshEditor(page, { e2e: true });
  await startBasicFace(page);
  await openCharacter(page);
  ...
  await page.locator('#face-pack-file').setInputFiles({ name: 'flat.json', mimeType: 'application/json', buffer: Buffer.from(JSON.stringify({ format: 'boop-face-pack', version: 1, id: 'flat', name: 'Flat', parts: [flat] })) });
  ...
  expect((await character(page)).categories.find((item) => item.id === 'head').assetId).toBe('head.oval', 'a part nobody restyled stays exactly as it was');

  // And back again: Soft Cartoon offers to bring the restyled part home, which
  // is the round trip that could not be done before MASC-08A.
  // One part to bring home and one already there, so nothing would be left
