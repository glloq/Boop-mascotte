...
test('@critical @smoke sample, preview and project download work', async ({ page }) => {
  ...
  const documentBefore=await page.evaluate(()=>window.__BOOP_E2E__.document()),sessionBefore=await page.evaluate(()=>window.__BOOP_E2E__.session()),diagnosticsBefore=await page.evaluate(()=>window.__BOOP_E2E__.diagnostics()),historyBefore=await page.evaluate(()=>window.__BOOP_E2E__.history()),revisionsBefore=await page.evaluate(()=>window.__BOOP_E2E__.documentRevisions()),versionBefore=await page.evaluate(()=>window.__BOOP_E2E__.documentVersionToken());
  ...
  const documentAfterNavigation=await page.evaluate(()=>window.__BOOP_E2E__.document()),sessionAfterNavigation=await page.evaluate(()=>window.__BOOP_E2E__.session());expect(documentAfterNavigation).toEqual(documentBefore);expect(sessionAfterNavigation).not.toEqual(sessionBefore);expect(sessionAfterNavigation).toMatchObject({workspace:'rig',activeSemanticPartId:'gaze'});
  ...
  const documentAfter=await page.evaluate(()=>window.__BOOP_E2E__.document()),diagnosticsAfter=await page.evaluate(()=>window.__BOOP_E2E__.diagnostics()),revisionsAfter=await page.evaluate(()=>window.__BOOP_E2E__.documentRevisions());expect(documentAfter).toEqual(documentBefore);expect(await page.evaluate(()=>window.__BOOP_E2E__.documentVersionToken())).toBe(versionBefore);expect(revisionsAfter).toEqual(revisionsBefore);expect(await page.evaluate(()=>window.__BOOP_E2E__.history())).toEqual(historyBefore);expect(diagnosticsAfter.store.documentMutations-diagnosticsBefore.store.documentMutations).toBe(0);expect(diagnosticsAfter.autosave.schedules-diagnosticsBefore.autosave.schedules).toBe(0);expect(diagnosticsAfter.autosave.writes-diagnosticsBefore.autosave.writes).toBe(0);expect(diagnosticsAfter.validation.runs-diagnosticsBefore.validation.runs).toBe(0);expect(await page.evaluate(()=>window.__BOOP_E2E__.dirty())).toBe(false);
