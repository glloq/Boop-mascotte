...
for(const [name,second] of [['Play/Pause','clip-pause'],['Play/Stop','clip-stop']])test(`@stability rapid ${name} remains single-loop`,async({page})=>{
  ...
  const d=await page.evaluate(()=>window.__BOOP_E2E__.diagnostics());expect(d.preview.activeRaf).toBe(0);await expect(page.locator('#clip-play')).toBeEnabled();
});
