...
export function createPreviewPanel(host, store, preview, { navigate = () => {}, readiness = () => null } = {}) {
  ...
  function render() {
    ...
    const activeReaction = preview.getActiveReaction?.()?.id || null;
    const reactions = (state.reactions || []).length ? `<section class="preview-section" data-preview-section="reactions"><h3>Reactions</h3><p class="small">Click the mascot to trigger its click reactions, or fire one here.</p><div class="chip-row">${state.reactions.map((item) => `<button type="button" class="chip${activeReaction === item.id ? ' chip-active' : ''}" data-preview-reaction="${esc(item.id)}" aria-pressed="${activeReaction === item.id}" title="${esc(triggerLabel(item.trigger))}">⚡ ${esc(item.name)}</button>`).join('')}</div></section>` : '';
