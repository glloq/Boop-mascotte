...
export function createPreviewPanel(host, store, preview, { navigate = () => {}, readiness = () => null } = {}) {
  ...
  host.addEventListener('click', (event) => {
    ...
    if (previewGo) { const model = readiness(); const target = model?.[previewGo]; if (target?.route) navigate(target.route); }
  });
  ...
  function render() {
    ...
    host.innerHTML = `<section class="preview-section" data-preview-section="live"><h3>Live controls</h3>${enabled.length ? `${pads}${sliders}<button type="button" class="secondary" data-preview-center>Center</button>` : '<p class="small">Turn on movements in Face Setup to test them live.</p>'}</section>${poses}${animations}${automatic}<section class="preview-section" data-preview-section="readiness"><h3>Ready?</h3><ol class="readiness-rows" aria-label="Project readiness">${rows}</ol></section>`;
  }
