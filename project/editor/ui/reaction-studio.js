...
export function createReactionStudio({ listHost, inspectorHost, store, history, preview, editorContext, onStatus = () => {}, navigate = () => {} }) {
  ...
  // The list is rebuilt on every edit; which groups are open must survive it.
  ...
  const inspectorSections = rememberOpen(inspectorHost);
  ...
  function renderList() {
    ...
    const presetSection = `${starterKitMarkup(starterKit.plan())}<section class="preset-catalogue" data-preset-catalogue="reactions"><h3>Ready-made reactions</h3>${presetGroups(reactionPresetAvailabilityGroups(state), card, { className: 'reaction-presets' })}</section>`;
