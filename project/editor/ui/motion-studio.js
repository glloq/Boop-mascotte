...
const summaryLine = (summary) => summary.kind === 'simple' ? `${summary.presetName} · ${summary.duration} s${summary.repeats > 1 ? ` · ×${summary.repeats}` : ''}${summary.loop ? ' · loop' : ''}`
  : summary.kind === 'edited' ? `${summary.presetName} · edited in the Timeline` : `Custom · ${plural(summary.tracks, 'track')} · ${plural(summary.keys, 'key')}`;
...
export function createMotionStudio({ listHost, inspectorHost, store, history, preview, editorContext, onStatus = () => {}, navigate = () => {}, openTimeline = () => {} }) {
  ...
  function renderList() {
    ...
    const items = clips.map((clip) => { const summary = motionSummary(state, clip); return `<li><button type="button" class="expression-item motion-item" data-motion-select="${esc(clip.id)}" data-motion-kind="${summary.kind}" aria-pressed="${clip.id === current}"><span>${esc(clip.name)}</span><small>${esc(summaryLine(summary))}</small></button></li>`; }).join('');
  ...
  function renderInspector() {
    ...
    const status = summary.kind === 'simple' ? `<p class="small" data-motion-status="simple">${esc(summary.presetName)} preset · ${summary.controls.map((name) => esc(controlLabel(name))).join(', ')}</p>`
      : summary.kind === 'edited' ? `<p class="face-pick-notice" data-tone="warn" data-motion-status="edited"><span>Edited in the Timeline: the ${esc(summary.presetName)} settings no longer drive this animation. Keep editing its keys below.</span></p>`
