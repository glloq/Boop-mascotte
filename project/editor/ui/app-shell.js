...
import { buildAddPartSection, buildPluginSection } from './sidebar-sections.js';
...
export function createAppShell(root) {
  ...
  root.innerHTML=`<a class="skip-link" href="#canvas">Skip to canvas</a><header class="topbar" aria-label="Project bar"><button id="drawer-toggle" class="drawer-toggle" aria-label="Tasks and tools" aria-expanded="false" aria-controls="left">☰</button><button id="home-button" class="brand-home" aria-label="Home">BOOP <span>Mascot Studio</span></button>
    ...
    <main class="workspace" aria-label="Workspace">
      <aside class="panel" id="left" aria-label="Tasks and tools"><button class="collapse-panel" id="collapse-left" aria-label="Collapse left panel">‹</button>
        <section class="character-tools"><h2>Character</h2><div class="workspace-hint" data-hint="character"></div><div id="part-browser"></div></section>
        ...
        <section class="create-tools"><h2>Artwork</h2><div class="workspace-hint" data-hint="create"></div><label class="button secondary artwork-import">Import / Replace SVG<input hidden type="file" id="artwork-svg-file" accept=".svg"></label>${gateMarkup('artwork', 'mobile')}<div id="artboard-panel"></div><details class="artwork-create"><summary>Add / Create artwork</summary><div class="template-cards"><button id="empty-basic"><b>◯ Start over with the Mascot Face</b><small>Replace the current artwork with the complete cartoon face template, rigged and turning in 2.5D.</small></button></div><div class="core-list"><h3>Ready</h3><div id="core-status"></div><button id="continue-rigging">Continue to Face Setup</button></div>${buildAddPartSection()}</details></section>
        ...
        <section class="animate-tools"><h2>Motions</h2><div class="workspace-hint" data-hint="animate"></div><div id="motion-panel"></div>${gateMarkup('timeline', 'mobile')}<details class="author-advanced" data-author-editor><summary><span class="setup-title">States &amp; behaviors</span><span class="setup-summary">advanced</span></summary>${gateMarkup('state-machine', 'mobile')}<div id="state-editor"></div></details></section>
  ...
  function applyWorkspace(name, emit=true) { if(!WORKSPACES.includes(name))return; const changed=preferences.workspace!==name; preferences.workspace=name; root.dataset.workspace=name; qAll('.workspace-tab').forEach(button=>{button.classList.toggle('active',button.dataset.workspace===name);button.setAttribute('aria-pressed',button.dataset.workspace===name);});
    // A column keeps its scroll position across a task change, so the panel
    // for the new task used to open scrolled halfway down whatever the last
    // one had been reading. Each task starts at the top of its own column.
  ...
  return {leftSidebarEl,canvasEl:q('#canvas'),inspectorEl:q('#inspector'),partBrowserEl:q('#part-browser'),partInspectorEl:q('#part-inspector'),rigEl:q('#rig-panel'),rigPartsEl:q('#rig-parts'),faceSetupEl:q('#face-setup-checklist'),faceMovementsEl:q('#face-movements'),gazePanelEl:q('#gaze-panel'),headPoseEl:q('#head-pose'),handSetupEl:q('#hand-setup'),warpPanelEl:q('#warp-panel'),holdingPanelEl:q('#holding-panel'),previewPanelEl:q('#preview-panel'),expressionsEl:q('#expressions-panel'),expressionInspectorEl:q('#expression-inspector'),motionsEl:q('#motion-panel'),automaticEl:q('#automatic-panel'),motionInspectorEl:q('#motion-inspector'),reactionsEl:q('#reactions-panel'),reactionInspectorEl:q('#reaction-inspector'),showTimeline(){if(preferences.timelineCollapsed){preferences.timelineCollapsed=false;root.classList.remove('timeline-collapsed');syncTimelineToggle();savePreferences();root.dispatchEvent(new CustomEvent('timelinetoggle',{detail:{open:true}}));}},
    ...
    getWorkspace:()=>preferences.workspace,homeEl:q('[data-home]'),contextInspectorEl:q('#context-inspector'),bindTaskNavigation(handler){taskNavigationHandler=handler;},
