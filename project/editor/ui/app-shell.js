  ...
  root.innerHTML=`<a class="skip-link" href="#canvas">Skip to canvas</a><header class="topbar" aria-label="Project bar"><button id="drawer-toggle" class="drawer-toggle" aria-label="Tasks and tools" aria-expanded="false" aria-controls="left">☰</button><button id="home-button" class="brand-home" aria-label="Home">BOOP <span>Mascot Studio</span></button>
    ...
    <main class="workspace" aria-label="Workspace">
      <aside class="panel" id="left" aria-label="Tasks and tools"><button class="collapse-panel" id="collapse-left" aria-label="Collapse left panel">‹</button>
        <section class="create-tools"><h2>Artwork</h2><div class="workspace-hint" data-hint="create"></div><label class="button secondary artwork-import">Import / Replace SVG<input hidden type="file" id="artwork-svg-file" accept=".svg"></label>${gateMarkup('artwork', 'mobile')}<div id="layers-panel"></div><details class="artwork-create"><summary>Add / Create artwork</summary><div class="template-cards"><button id="empty-basic"><b>◯ Start with Basic Face</b><small>A simple mascot you can extend step by step.</small></button></div><div class="core-list"><h3>Ready</h3><div id="core-status"></div><button id="continue-rigging">Continue to Face Setup</button></div><div class="feature-list"><h3>Add expression</h3><article class="feature-card"><div><b>Eyebrows</b><small>Curious and angry expressions</small></div><button data-add-feature="eyebrows">+ Add</button></article><article class="feature-card"><div><b>Eyelids</b><small>More natural blinking and eye expressions</small></div><button data-add-feature="eyelids">+ Add</button></article><p class="feature-incompatible small" hidden>Preset artwork is only added to compatible starter faces. Use Face Setup to configure artwork in an imported SVG.</p></div><details class="more-examples"><summary>More templates and tools</summary><button id="empty-expressive">Expressive Face</button><button id="empty-talking">Talking Face</button>${buildFaceBuilderSection()}${buildPresetSection()}</details></details></section>
        ...
        <section class="animate-tools"><h2>Animate</h2><div class="workspace-hint" data-hint="animate"></div><div id="motion-panel"></div><div id="automatic-panel"></div>${gateMarkup('timeline', 'mobile')}${gateMarkup('state-machine', 'mobile')}</section>
        <section class="reactions-tools"><h2>Reactions</h2><div class="workspace-hint" data-hint="reactions"></div><div id="reactions-panel"></div></section>
