  ...
  function render() {
    ...
    if (tool === 'select' && selection) {
      ...
      if (ids.length) {
        ...
        ].map(([what, label, title]) => item('align', what, label, title)).join('')}<span>Spread</span>${item('distribute', 'horizontal', '↔', 'Equal gaps left to right (three or more pieces)', ids.length > 2)}${item('distribute', 'vertical', '↕', 'Equal gaps top to bottom (three or more pieces)', ids.length > 2)}${vector ? `${item('group', 'selection', 'Group', 'Make the selected pieces one group (Ctrl/Cmd+G)', many)}${item('clip', 'selection', 'Cut to top', 'Cut the pieces to the one in front: to its outline if it is a shape, to where it is not transparent if it is a picture. It stops being drawn and does the cutting', many)}` : ''}</span>`);
