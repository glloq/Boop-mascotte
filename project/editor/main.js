...
if (new URLSearchParams(location.search).has('e2e')) {
  ...
  window.__BOOP_E2E__ = {
    ...
    controlState: (name) => {
      ...
      const compiled=compileFrame(store.getState().elements,effective,store.getState().globalConstraints,store.getState().stateConstraints?.[store.getState().activeState],{keyforms:store.getState().keyforms,shapeKeys:store.getState().shapeKeys,hands:store.getState().hands});
    ...
    frameFor: (id) => {
      ...
      const compiled=compileFrame(state.elements,effective,state.globalConstraints,state.stateConstraints?.[state.activeState],{keyforms:state.keyforms,shapeKeys:state.shapeKeys,hands:state.hands});
