...
export function wireOverlays({ root, q, qAll }) {
  ...
  /**
   * And how long one carrying a button stays, because the button has to be
   * *reachable* and not only visible: 2.6 s is long enough to read "Left eye
   * deleted" and much too short to decide about it, move a pointer across a
   * canvas and press.
   */
  const ACTION_HOLD = 7000;
  ...
  return {
    homeEl: q('[data-home]'),
    // The three screens Home's New Character card opens (ui/character-builder/create-wizard.js).
    wizardEl: q('#create-wizard'),
    ...
    /**
     * Say something, and offer the one thing to do about it.
     *
     * `action` is why this exists. Fifteen messages in the editor ended "Undo
     * puts it back", written for a toast with a button and shipped into one
     * that wrote `textContent` — so the sentence named a keystroke that does
     * not exist on a phone and offered nothing to press anywhere. A toast
     * carrying an action holds for `ACTION_HOLD`, takes pointer events (the
     * stylesheet keys that off `data-actionable`), and clears itself the moment
     * the action runs, because a button offering to undo something already
     * undone is worse than no button.
     *
     * @param {string} message
     * @param {'info'|'warn'|'error'} [tone]
     * @param {{ routine?: boolean, action?: { label: string, run: () => void } }} [options]
     */
    setStatus(message, tone = 'info', { routine = false, action = null } = {}) {
      const el = q('#toast'), now = Date.now();
      // A routine message is the heartbeat -- the validation pass, the ready
      // line -- and it never outranks something the author just caused. The
      // tone does not change that: this used to read `tone === 'info'`, so the
      // validation summary, which is a warning, wiped the toast offering Undo
      // on the edit that had triggered the pass. The offer was gone before it
      // could be taken.
      if (routine && !action && now < statusHeldUntil) return;
      const hold = action ? ACTION_HOLD : STATUS_HOLD;
      ...
      if (!action) {
        ...
        statusHeldUntil = routine ? 0 : now + hold;
        if (tone === 'info') toastTimer = setTimeout(() => el.classList.remove('visible'), hold);
        return;
      }
      el.dataset.actionable = 'true';
      el.innerHTML = `<span data-toast-message></span><button type="button" data-toast-action></button>`;
      el.querySelector('[data-toast-message]').textContent = message;
      const button = el.querySelector('[data-toast-action]');
      button.textContent = action.label;
      button.setAttribute('aria-label', `${action.label}: ${message}`);
      button.onclick = () => {
        clearTimeout(toastTimer);
      ...
      statusHeldUntil = now + hold;
      toastTimer = setTimeout(() => { el.classList.remove('visible'); delete el.dataset.actionable; }, hold);
    },
    exitFocus(){root.classList.remove('focus-preview');},isFocus(){return root.classList.contains('focus-preview');},
    showProblems(readiness,issues,onFix,onGo,onRepair){problemsOpener=document.activeElement&&document.activeElement!==document.body?document.activeElement:q('#validate');const panel=q('#problems-panel'),symbol={ready:'✓',warning:'⚠',error:'●',todo:'○',optional:'·'},actionable=issues.filter(issue=>issue.severity!=='info');panel.dataset.projectCheckStatus=actionable.some(x=>x.severity==='error')?'error':actionable.length?'warning':'ready';panel.hidden=false;panel.innerHTML=`<div class="card-title"><h3>Project check</h3><button class="icon" data-close-problems aria-label="Close Problems">×</button></div><ol class="readiness-rows readiness-list" aria-label="Project readiness">${groupReadiness(readiness).map(group=>`<li class="readiness-group" data-readiness-group="${group.id||'export'}" data-readiness-status="${group.status}"><span class="readiness-symbol" aria-hidden="true">${symbol[group.status]||'○'}</span><b>${esc(group.label)}</b></li>${group.sections.map(item=>`<li data-readiness-section="${item.id}" data-readiness-status="${item.status}"><span class="readiness-symbol" aria-hidden="true">${symbol[item.status]||'○'}</span><span class="readiness-copy"><b>${esc(item.label)}</b><small>${esc(item.summary)}</small></span>${item.route?`<button type="button" class="secondary" data-readiness-go="${item.id}" aria-label="Go to ${esc(item.label)}">${item.action?'Fix':'Go'}</button>`:''}</li>`).join('')}`).join('')}</ol>${actionable.length?`<p>${actionable.length} thing${actionable.length===1?'':'s'} need attention</p>${actionable.map((issue,index)=>`<article class="manager-card" data-diagnostic-id="${esc(issue.id)}"><b>${issue.severity==='error'?'● Error':'⚠ Warning'}</b><p>${esc(issue.message)}</p><small>${esc(describeRemedy(issue).available?describeRemedy(issue).explanation:describeFix(issue).explanation)}</small><div class="problem-actions">${issue.fix?`<button class="secondary" data-fix-problem="${index}">Fix</button>`:''}${describeRemedy(issue).available?`<button data-repair-problem="${index}">${esc(describeRemedy(issue).label)}</button>`:''}</div></article>`).join('')}`:'<p class="ready-message">✓ No problems found</p>'}`;panel.onclick=event=>{if(event.target.dataset.closeProblems!==undefined)panel.hidden=true;if(event.target.dataset.fixProblem!==undefined){panel.hidden=true;onFix?.(actionable[Number(event.target.dataset.fixProblem)]);}if(event.target.dataset.repairProblem!==undefined){panel.hidden=true;onRepair?.(actionable[Number(event.target.dataset.repairProblem)]);}if(event.target.dataset.readinessGo){panel.hidden=true;onGo?.(readiness[event.target.dataset.readinessGo]);}};panel.querySelector('[data-close-problems]')?.focus();},
    ...
    /**
     * Ask before a delete whose shape is hard to picture (`deleteConfirmation`).
     *
     * Only ever called for the three cases that earn it — several pieces at
     * once, a piece other parts hang on, a group carrying the rig — because a
     * dialog in front of every delete teaches people to dismiss dialogs, and
     * the toast's Undo already covers the ordinary case.
     */
    confirmDelete({ question, detail } = {}) {
      const dialog = q('#delete-dialog'), returnFocus = document.activeElement;
      dialog.querySelector('[data-delete-question]').textContent = question || 'Delete this?';
      dialog.querySelector('[data-delete-detail]').textContent = detail || '';
      return new Promise((resolve) => {
        dialog.addEventListener('close', () => { returnFocus?.focus?.(); resolve(dialog.returnValue === 'delete'); }, { once: true });
        dialog.showModal();
