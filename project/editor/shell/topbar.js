...
export const topbarMarkup = () => `<a class="skip-link" href="#canvas">Skip to canvas</a><header class="topbar" aria-label="Project bar"><button id="drawer-toggle" class="drawer-toggle" aria-label="Tasks and tools" aria-expanded="false" aria-controls="left">☰</button><button id="home-button" class="brand-home" aria-label="Home">BOOP <span>Mascot Studio</span></button>
    ${workspaceNavMarkup()}
    <nav class="project-actions"><button id="search-button" aria-label="Search actions and items (Ctrl+K)" title="Search (Ctrl+K)">🔍</button><button id="undo" aria-label="Undo">↶</button><button id="redo" aria-label="Redo">↷</button><button id="validate" aria-label="Project check" title="Check project readiness">Project check</button><button id="reset-mascot-top" class="secondary reset-mascot" aria-label="Reset mascot" title="The face back to rest, and every preview-only change cleared. Nothing in your project changes. (Ctrl/Cmd+Alt+R)">⟲ Rest</button><button id="save-project-top" aria-label="Save Project" title="Keeps your editable Boop project">Save Project</button><button id="export-top" data-action="open-export" title="Creates files for using the mascot outside the editor">Export</button><details class="file-menu"><summary aria-label="More project actions">•••</summary><div class="menu-popover"><button id="new-project">New Project</button><button id="recover-autosave" hidden>Recover local draft</button><label class="button secondary">Open Project <small>A .boop package or a .json snapshot</small><input hidden type="file" id="project-file" accept=".boop,.json"></label><label class="button secondary">Import SVG <small>Artwork only</small><input hidden type="file" id="svg-file" accept=".svg"></label><label class="button secondary">Import rig.json <small>Rig data onto the current artwork</small><input hidden type="file" id="rig-file" accept=".json,application/json"></label><label class="button secondary">Import face pack <small>Library parts and presets, kept in this browser</small><input hidden type="file" id="face-pack-file" accept=".json,application/json"></label><button type="button" id="simple-mode-toggle" class="secondary" aria-pressed="false">Simple editor<small>Just the mascot: Design and Preview</small></button><button type="button" id="capability-toggle" class="secondary capability-toggle">What works on this screen</button><div class="compact-only menu-group" role="group" aria-label="Actions"><button type="button" id="menu-undo" class="secondary">↶ Undo</button><button type="button" id="menu-redo" class="secondary">↷ Redo</button><button type="button" id="menu-problems" class="secondary">Project check</button><button type="button" id="menu-search" class="secondary">🔍 Search</button></div><details><summary>Advanced</summary><button type="button" class="secondary" data-open-advanced>Advanced tools…</button>${buildPluginSection()}</details></div></details></nav><span id="save-state" class="status-pill">✓ Saved</span></header>`;
...
export function wireTopbar({ root, q }) {
  ...
  return {
    ...
    /**
     * The readiness of the whole project, in the bar, on every screen (UIR-14).
     *
     * "Problems" was a word that was true whether or not there were any. The
     * button reads the project instead: ✓ Ready, or how many things want
     * attention, which is also what makes it worth pressing.
     */
    setReadinessVerdict({ status, label }) {
      const button = q('#validate');
      button.textContent = label;
      button.dataset.readiness = status;
      button.setAttribute('aria-label', `Project check: ${label.replace(/^[^\w]+/, '')}`);
    },
    ...
    bindCapabilities(handler) { q('#capability-toggle').onclick = () => { closeMenu(); handler(); }; },
    /**
     * Fold the three rigging workspaces away, or bring them back (audit §7.3).
     *
     * A label that says what pressing it *gets* you, not what mode you are in:
     * "Simple editor" turns it on, "Every workspace" turns it off, and
     * `aria-pressed` carries the state for anybody reading it aloud.
     */
    bindSimpleMode(handler) { q('#simple-mode-toggle').onclick = () => { closeMenu(); handler(); }; },
    setSimpleModeLabel(on) {
      const button = q('#simple-mode-toggle');
      button.setAttribute('aria-pressed', String(Boolean(on)));
      button.innerHTML = on
        ? 'Every workspace<small>Rig, Animate and Behavior as well</small>'
        : 'Simple editor<small>Just the mascot: Design and Preview</small>';
    },
