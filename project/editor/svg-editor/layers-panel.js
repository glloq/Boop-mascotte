...
export function createLayersPanel(leftSidebarEl, store, history, canvas) {
  ...
  function row(item, depth) {
    ...
    return `<div role="treeitem" aria-level="${depth + 1}" aria-selected="${selected}" ${item.children.length?`aria-expanded="${expanded}"`:''} tabindex="${focusedId === item.id || (!focusedId && selected) ? '0' : '-1'}" data-layer-id="${escapeHtml(item.id)}" class="layer-item ${selected?'active':''}" style="margin-left:${depth*12}px"><div class="layer-row">${item.children.length?`<button class="layer-icon" tabindex="-1" data-action="toggle" data-id="${escapeHtml(item.id)}" aria-label="${expanded?'Collapse':'Expand'} ${escapeHtml(item.name)}">${expanded?'▼':'▶'}</button>`:'<span class="layer-spacer"></span>'}<button class="layer-label" tabindex="-1" data-action="select" data-id="${escapeHtml(item.id)}"><small>[${escapeHtml(item.type[0].toUpperCase())}]</small> ${escapeHtml(item.name)} ${part?`<span class="semantic-badge">${escapeHtml(part.name)}</span>`:''}</button><button class="layer-icon" tabindex="-1" data-action="visibility" data-id="${escapeHtml(item.id)}" title="Visibility">${item.visible?'◉':'○'}</button><button class="layer-icon" tabindex="-1" data-action="lock" data-id="${escapeHtml(item.id)}" title="Lock">${metadata.locked?'🔒':'🔓'}</button><button class="layer-icon" tabindex="-1" data-action="up" data-id="${escapeHtml(item.id)}">↑</button><button class="layer-icon" tabindex="-1" data-action="down" data-id="${escapeHtml(item.id)}">↓</button></div>${selected?`<input data-action="rename" data-id="${escapeHtml(item.id)}" aria-label="Layer display name" value="${escapeHtml(item.name)}"><div class="layer-actions"><button data-action="duplicate" data-id="${escapeHtml(item.id)}">Duplicate</button><button data-action="${item.type==='g'?'ungroup':'group'}" data-id="${escapeHtml(item.id)}">${item.type==='g'?'Ungroup':'Group'}</button><button class="danger" data-action="delete" data-id="${escapeHtml(item.id)}">Delete</button></div><div class="small">ID: ${escapeHtml(item.id)}</div>`:''}${expanded?item.children.map(child=>row(child,depth+1)).join(''):''}</div>`;
  }
