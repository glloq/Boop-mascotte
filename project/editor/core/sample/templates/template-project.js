...
export function applyTemplateProject(state, kind) {
  ...
  if(kind==='builder'){
    add(state,'eyes',{leftEye:'eyeLeft',rightEye:'eyeRight'},['eyeOpen']);add(state,'mouth',{mouth:'mouth'},['mouthOpen','smile','mouthWidth']);state.behaviors=[];state.animationClips=[clips.smile];
  ...
  else if(kind!=='builder') {state.behaviors=[];state.animationClips=[clips.look,clips.smile];}
