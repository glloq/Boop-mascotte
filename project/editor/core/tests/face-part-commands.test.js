...
test('replacing a mouth is one write and one undo step, and the canvas is asked once', () => {
  ...
  assert.deepEqual([result.partId, result.rootId, result.ids, result.enabled, result.disabled, result.fitted], ['mouth', 'mouth-full', ['mouth-full', 'mouth', 'mouth-full-inside', 'tongue', 'teethLower', 'teeth', 'tongueTip', 'tongueGroove'],
  ...
  assert.deepEqual(ui.canvas.calls.replace[0].removeIds, ['mouth', 'tongueTip', 'tongueGroove', 'mouthInside', 'tongue', 'teethLower', 'teeth']);
  ...
  assert.deepEqual(Object.values(after.semanticParts).find((part) => part.type === 'mouth').roles,
    { mouth: 'mouth', teeth: 'teeth', teethLower: 'teethLower', tongue: 'tongue', tongueTip: 'tongueTip', tongueGroove: 'tongueGroove' });
