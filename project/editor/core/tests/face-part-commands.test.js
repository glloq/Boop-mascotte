...
test('replacing a mouth is one write and one undo step, and the canvas is asked once', () => {
  ...
  assert.deepEqual([result.partId, result.rootId, result.ids, result.enabled, result.disabled, result.fitted], ['mouth', 'mouth-full', ['mouth-full', 'mouth', 'mouth-full-inside', 'uvula', 'tongue', 'teethLower', 'teeth', 'tongueTip', 'tongueGroove'],
      ['mouthOpen', 'smile', 'mouthWidth', 'mouthRound', 'mouthSkew', 'teeth', 'tongue', 'uvula', 'tongueX', 'tongueY', 'tongueOut', 'tongueCurl'], [], true]);
  ...
  assert.deepEqual(ui.canvas.calls.replace[0].removeIds, ['mouth', 'tongueTip', 'tongueGroove', 'mouthInside', 'uvula', 'tongue', 'teethLower', 'teeth']);
  ...
  assert.deepEqual(Object.values(after.semanticParts).find((part) => part.type === 'mouth').roles,
    { mouth: 'mouth', uvula: 'uvula', teeth: 'teeth', teethLower: 'teethLower', tongue: 'tongue', tongueTip: 'tongueTip', tongueGroove: 'tongueGroove' });
