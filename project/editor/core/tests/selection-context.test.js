...
test('inspector presentation supports task-level Face Setup onboarding', () => {
  ...
  assert.deepEqual(resolveSelectionContext({selectedId:'mouth'},'character'),{kind:'artwork',id:'mouth'},'an older name for the surface still resolves');
  assert.equal(resolveInspectorPresentation('artwork',{kind:'artwork',id:'mouth'}).artwork,true);
