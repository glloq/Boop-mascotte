...
test('the setup steps say what to do next, in order', () => {
  ...
  assert.match(handSetupSteps({ element: 'h', parent: 'body', anchor: { x: 1, y: 1 } }, { h: {} }).next, /Give it drawings/);
  assert.equal(handSetupSteps({ element: 'h', parent: 'body', anchor: { x: 1, y: 1 }, styles: { library: [{ id: 'open' }] } }, { h: {} }).done, 4);
  assert.match(handSetupSteps({ element: 'gone', parent: 'body', anchor: { x: 1, y: 1 } }, {}).next, /no longer exists/);
});
...
test('a hand with no drawings yet is offered them, and has no row to press', () => {
  const it = harness();
  it.change({ handField: 'artwork', handSide: 'left' }, 'handLeft');
...
test('mirroring needs a hand first, then fills in the other side', () => {
  ...
  assert.equal(it.hands().right.styles, undefined, 'each hand holds its own drawings');
  assert.ok(it.params().handRX && it.params().handRRotation, 'the other side gets its movements too');
});
