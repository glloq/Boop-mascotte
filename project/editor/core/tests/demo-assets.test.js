...
test('the template artwork parses into the records the canvas would build', () => {
  ...
  assert.equal(Object.keys(elements).length, 35, 'every layer the artwork draws, and nothing under <defs>');
...
test('the template export is the rig the editor writes for the untouched face', () => {
  ...
  assert.equal(Object.keys(rig.elements).length, 35);
  ...
  assert.deepEqual(rig.followers.map((follower) => follower.element), ['earLeft', 'earRight', 'hair', 'hairTop', 'hairBack'], 'the ears and the hair trail the head');
