...
import {
  ...
  generateHeadTurn, headTurnCellSamples, headTurnElements, headTurnKeyforms, headTurnTravel, headTurnUnit
...
test('a turn is built from the face parts, and only the ones that belong to a head', () => {
  ...
  assert.equal(byElement.face.depth, 0, 'the outline is moved by its own binding, not by the grid');
...
test('the turn is measured from the head, and falls back to what the movement travels', () => {
  ...
  assert.equal(headTurnUnit(document, { headWidth: 200 }), 10, 'about 5% of the head width');
  ...
  assert.equal(headTurnUnit(document, { headWidth: 4000 }), 40, 'clamped, so a huge canvas cannot fling the features');
...
test('looking up and down moves the features with the head and compresses it', () => {
  ...
  assert.equal(up.nose.scaleY, undefined, 'only the outline is compressed');
  const right = turn.cells.find((cell) => cell.x === 1 && cell.y === 0).samples;
...
test('a feature that is not inside the head carries the head movement itself', () => {
  ...
  assert.equal(nose(nestedTurn), 10, 'drawn inside the head group, it inherits the 8 the head travels');
  assert.equal(nose(flatTurn), 18, 'drawn beside it, it has to travel that 8 itself');
...
test('strength scales the whole effect and stays inside what a transform can mean', () => {
  ...
  assert.deepEqual(at(0).nose, { translateX: 0, translateY: 0 }, 'no strength, no turn');
...
test('a generated turn is ordinary keyforms: the runtime turns the head with no head-pose code', () => {
  ...
  // The head travels 8 through its binding; the nose travels further still.
  assert.equal(turned.face.transform.x, 8);
...
test('a scale is only generated where the part was measured, and it holds its centre', () => {
  ...
  assert.deepEqual(blind.face, {}, 'nothing to squash around either');
