...
import { normalizeRigging } from './rigging-types.js';
...
export function normalizeRig(raw = {}) {
  ...
  const elements = Object.fromEntries(Object.entries(raw.elements || {}).map(([id, source]) => {
    ...
    return [id, { ...source,
      ...
      // How this piece moves (V5-02, core/rig/rigging-types.js). Every element
      // written before it reads as `rigid`, which is what they all were.
      rigging: normalizeRigging(source.rigging, source.meta?.nodeType),
  ...
  return { ...raw, schemaVersion: RIG_SCHEMA_VERSION, params, states, elements, activeState, transitions, behaviors: normalizeBehaviors(raw), transitionSettings,
    ...
    // Pieces that are several drawings with one showing (V5-01). Empty for
    // every mascot that is one drawing per piece, which is every older one.
    partStates: normalizePartStates(raw),
