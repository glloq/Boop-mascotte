import { RIG_SCHEMA_VERSION, BINDING_PROPERTIES, normalizeBinding, normalizeBehaviors, normalizeKeyforms, normalizeShapeKeys, normalizeHands, normalizeDeformers, normalizeParallax, normalizeExpressionBlend, normalizeWarps, clampDepth, normalizeMotionBlend } from '../../../runtime/runtime.js';
...
export function normalizeRig(raw = {}) {
  ...
  const elements = Object.fromEntries(Object.entries(raw.elements || {}).map(([id, source]) => {
    ...
    return [id, { ...source,
      ...
      // Hierarchy membership (docs/DEFORMER_MODEL.md); absent on plain elements.
      ...(typeof source.deformer === 'string' && source.deformer ? { deformer: source.deformer } : {}),
      // Pseudo depth (docs/DEPTH_PARALLAX.md); absent means flat.
      ...(Number.isFinite(Number(source.depth)) ? { depth: clampDepth(source.depth) } : {}),
  ...
  return { ...raw, schemaVersion: RIG_SCHEMA_VERSION, params, states, elements, activeState, transitions, behaviors: normalizeBehaviors(raw), transitionSettings,
    ...
    keyforms: normalizeKeyforms(raw), shapeKeys: normalizeShapeKeys(raw), hands: normalizeHands(raw), deformers: normalizeDeformers(raw), parallax: normalizeParallax(raw.parallax), expressionBlend: normalizeExpressionBlend(raw.expressionBlend), motionBlend: normalizeMotionBlend(raw.motionBlend), warps: normalizeWarps(raw),
