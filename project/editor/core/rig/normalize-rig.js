import { RIG_SCHEMA_VERSION, BINDING_PROPERTIES, normalizeBinding, normalizeBehaviors, normalizeKeyforms, normalizeShapeKeys, normalizeHands, normalizeDeformers, normalizeParallax, normalizeExpressionBlend, clampDepth } from '../../../runtime/runtime.js';
...
export function normalizeRig(raw = {}) {
  ...
  return { ...raw, schemaVersion: RIG_SCHEMA_VERSION, params, states, elements, activeState, transitions, behaviors: normalizeBehaviors(raw), transitionSettings,
    ...
    keyforms: normalizeKeyforms(raw), shapeKeys: normalizeShapeKeys(raw), hands: normalizeHands(raw), deformers: normalizeDeformers(raw), parallax: normalizeParallax(raw.parallax), expressionBlend: normalizeExpressionBlend(raw.expressionBlend),
