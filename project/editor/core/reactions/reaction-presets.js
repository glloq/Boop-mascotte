...
export const REACTION_PRESETS = Object.freeze([
  ...
  reaction('When clicked', 'surprise', 'Surprise', 'Clicked \u2192 a surprised face and a quick pop, then back.', click, 'fast', ['surprised', 'excited'], ['head-pop', 'gasp', 'bounce', 'nod']),
  reaction('When clicked', 'greet', 'Greet', 'Clicked \u2192 a happy face, a nod and a wave.', click, 'normal', ['happy', 'excited'], ['nod', 'bounce'], ['wave', 'open']),
  reaction('When clicked', 'laugh', 'Laugh', 'Clicked \u2192 laughing, with the head bobbing.', click, 'normal', ['laughing', 'excited', 'happy'], ['laugh', 'bounce', 'nod']),
  reaction('When clicked', 'shy', 'Shy', 'Clicked \u2192 looks away, then comes back.', click, 'slow', ['shy', 'sad', 'confused', 'sleepy'], ['shake', 'tilt', 'wobble']),
  reaction('When clicked', 'grumble', 'Grumble', 'Clicked \u2192 unimpressed, with a slow head shake.', click, 'normal', ['annoyed', 'angry', 'sulking'], ['shake', 'wobble']),
  reaction('When clicked', 'cheer', 'Cheer', 'Clicked \u2192 excited, bouncing, hand up.', click, 'fast', ['excited', 'proud', 'happy'], ['bounce', 'head-pop', 'laugh'], ['thumbsUp', 'wave', 'open']),
  ...
  reaction('From your page', 'say-no', 'Say no', "mascot.trigger('custom', { name: 'no' }) \u2192 a firm head shake.", custom('no'), 'fast', ['sad', 'annoyed', 'angry'], ['shake', 'wobble']),
  reaction('From your page', 'celebrate', 'Celebrate', "On 'success' \u2192 excited, bouncing, hand up.", custom('success'), 'normal', ['excited', 'proud', 'happy'], ['bounce', 'head-pop', 'laugh'], ['thumbsUp', 'wave', 'peace']),
  reaction('From your page', 'oops', 'Oops', "On 'error' \u2192 worried, with a small shiver.", custom('error'), 'fast', ['worried', 'scared', 'confused'], ['shiver', 'shake', 'gasp']),
  reaction('From your page', 'ponder', 'Ponder', "On 'thinking' \u2192 looks away and thinks, and stays that way.", custom('thinking'), 'slow', ['thinking', 'confused', 'curious'], ['look-up', 'tilt', 'look-around'], null, 'stay')
]);
...
export function instantiateReactionPreset(document = {}, preset) {
  ...
  if (wanted.length && !gestures.length) missing.push({ kind: 'gesture', label: `a ${wanted[0]} hand pose`, route: { task: 'face-setup', focus: 'hand-setup' } });
