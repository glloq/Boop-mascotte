...
const WORKSPACE_LABELS = { create: 'Artwork', rig: 'Face Setup', expressions: 'Expressions', animate: 'Animate', reactions: 'Reactions', preview: 'Preview' };
const AUTHOR_MODE_LABELS = { animations: 'Timeline', states: 'States', behaviors: 'Behaviors (advanced)' };
...
export function describeFix(issue) {
  ...
  const detail = fix.authorMode ? ` → ${AUTHOR_MODE_LABELS[fix.authorMode] || fix.authorMode}` : fix.rigTask === 'calibrate' ? ' → Calibrate' : '';
