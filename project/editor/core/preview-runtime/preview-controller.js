...
export function createPreviewController({ store, canvas, requestFrame = requestAnimationFrame, cancelFrame = cancelAnimationFrame, now = () => performance.now(), onFrame = () => {}, onError = () => {} }) {
  ...
  const api={
    ...
    getCurrentTime:()=>clipTime,getPreviewElapsed:()=>previewElapsed,getTransitionElapsed:()=>transitionElapsed,getEffectiveParams:()=>({...effective}),isRunning:()=>running,isPlaying:()=>playing,getLastError:()=>lastError,
