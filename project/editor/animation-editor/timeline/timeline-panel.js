...
export function createTimelinePanel(host,store,history,preview,editorContext=null,notify=()=>{}){
  ...
  host.addEventListener('click',event=>{const button=event.target.closest('button');const clip=active();
    ...
    if(button?.id==='clip-play')playTransport();
    if(button?.id==='clip-pause')pauseTransport();
    ...
    // One zoom control instead of four: ctrl/cmd+wheel already scales the sheet
    // continuously, and Fit answers the question a pair of steppers cannot --
    // show me the whole motion.
    if(button?.dataset.zoom==='fit'&&clip){const width=host.querySelector('.dope-viewport')?.clientWidth||500;ui.zoom=Math.max(.25,Math.min(8,width/Math.max(1,clip.duration*160)));ui.scrollLeft=0;render();}
  });
  ...
  /**
   * Play, pause and stop act on the transport that is actually playing.
   *
   * `preview.isPlaying()` answers *is anything moving* -- the clip scrub, an
   * arrangement, or a motion on the shared layer -- so asking it and then
   * calling `pauseClip` left Space and the Pause button dead whenever a motion
   * or an arrangement was the thing running: `pauseClip` returns false unless
   * the clip transport itself was started. An arrangement and a layered motion
   * have no pause of their own, because they are schedules rather than a
   * playhead, so the honest answer for those two is to stop them.
   */
  function playTransport(){const clip=active();preview.setClip(clip?.id||null);preview.seek(playhead());preview.playClip();if(ui.view==='arrangement')render();}
  function pauseTransport(){
    const stopped=preview.isArrangementPlaying?.()?preview.stopArrangement():preview.isMotionPlaying?.()?preview.stopMotion():preview.pauseClip();
    if(ui.view==='arrangement')render();
    return stopped;
  }
  function togglePlayback(){return preview.isPlaying()?pauseTransport():playTransport();}
  /**
   * Leaving the timeline stops what the timeline started.
   *
   * Stop was the only control that cleared the motion layer and the arrangement
   * from this side (`preview.stopClip`), so with the button gone that call lives
   * at the moment it was really for: switching away from the workspace the
   * transport is in. The authored playhead is put back afterwards -- a view
   * switch is not a rewind.
   */
  function stopPlayback(){if(!preview.isPlaying())return false;const at=playhead();preview.stopClip();preview.seek(at);return true;}
...
const keyEl=event.target.closest('[data-key]'),lane=event.target.closest('.key-lane'),ruler=event.target.closest('.time-ruler');
   ...
   *
   * It reports *why* it kept nothing rather than only that it did. Posing on
   * the canvas is how a key is made now (V3-13), and a gesture that silently
   * writes nothing is a gesture the author repeats. Whether the reason is worth
   * saying is the caller's: in Animate the drag was authoring, while in Preview
   * or Face Setup the same drag is only trying the mascot on.
   *
   * @returns {{keyed:boolean,reason:null|'nothing-moved'|'no-clip'|'auto-key-off',message:string|null}}
   */
  function autoKeyMany(values,{snapshot=true}={}){
    ...
    if(!names.length)return {keyed:false,reason:'nothing-moved',message:null};
    if(!clip)return {keyed:false,reason:'no-clip',message:'That pose was not keyed: no motion is open. Make one, then pose the mascot again.'};
    if(!session.animationEditor.autoKey)return {keyed:false,reason:'auto-key-off',message:'That pose was not keyed: Auto Key is off. Turn it on and the next one lands at the playhead.'};
    const time=playhead();
    ...
    return {keyed:true,reason:null,message:null};
  }

  return {
    syncArrangementPlayhead,render,requestRender,autoKey,autoKeyMany,togglePlayback,stopPlayback,reset};
}
