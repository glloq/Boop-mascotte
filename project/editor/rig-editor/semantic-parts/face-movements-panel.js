...
export function createFaceMovementsPanel(host, store, history, editorContext, { openMovement = () => {}, applyPose = () => {}, liveValues = () => ({}) } = {}) {
  ...
  function render() {
    ...
    const groups = [...checklist.bands].map(([band, parts]) => `<li class="movement-band" data-movement-band="${esc(band)}"><b class="movement-band-name">${esc(band)}</b>${[...parts].map(([group, items]) => `<div class="movement-group" data-movement-group="${esc(group)}">${parts.size > 1 || group !== band ? `<small class="movement-group-name">${esc(group)}</small>` : ''}${posesFor(items, live)}<ul class="movement-list">${rows(group, items)}</ul></div>`).join('')}</li>`).join('');
