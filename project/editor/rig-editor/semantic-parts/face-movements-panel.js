...
export function createFaceMovementsPanel(host, store, history, editorContext, { openMovement = () => {} } = {}) {
  ...
  function render() {
    ...
    host.innerHTML = `<div class="face-progress"><h3 id="face-movements-heading">Movements</h3><span class="status-pill">${checklist.enabled} / ${checklist.available} on</span></div><div role="status" aria-live="polite">${notice ? `<p class="face-pick-notice" data-tone="${notice.tone}">${esc(notice.text)}</p>` : ''}</div>${checklist.available ? '' : '<p class="small">Assign face parts above to unlock their movements.</p>'}<ul class="movement-groups" aria-labelledby="face-movements-heading">${groups}</ul>${offCount ? `<button type="button" class="face-next secondary" data-movement-enable-all>Turn on ${offCount === 1 ? 'the remaining movement' : `all ${offCount} available movements`}</button>` : ''}`;
